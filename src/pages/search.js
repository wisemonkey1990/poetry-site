...
export async function renderSearch(params, query) {
  ...
  const html = `<div class="search-shell">
    ...
    ${keyword ? `<section class="search-results"><p class="result-count">“${escapeHtml(keyword)}” · 共寻得 ${results.length} 篇</p>${results.length ? `<div class="search-result-list">${results.map((poem) => `<a href="#/poem/${poem.id}" class="card poem-card" data-nav="/poem/${poem.id}"><div class="poem-card-header"><h4>${toTitleRubyHTML(poem.title)}</h4><span class="tag">${poem.chapter} · ${poem.section}</span></div><p class="poem-excerpt">${highlightMatch(poem.content[0], keyword)}</p></a>`).join("")}</div>` : `<div class="no-result">未寻得相合诗篇，请换一词再试。</div>`}</section>` : `<div class="search-hint"><p>轻启墨卷，以一字寻一诗。</p></div>`}
  </div>`;
